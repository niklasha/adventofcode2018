package se.hallqvist.adventofcode2018.day22
...
class Day22 extends FlatSpec {
  ...
  val memo = mutable.Map[(I, I), V]()

  def index(c: (I, I), d: I, t: (I, I)): V = memo.getOrElse(c, {
    val rv: V = if (c == (0, 0) || c == t) 0
      else if (c._2 == 0) c._1 * 16807
      else if (c._1 == 0) c._2 * 48271
      else level((c._1 - 1, c._2), d, t) * level((c._1, c._2 - 1), d, t)
    memo(c) = rv
    rv
  })

  def level(c: (I, I), d: I, t: (I, I)) = (index(c, d, t) + d) % 20183

  def rtype(c: (I, I), d: I, t: (I, I)) = (level(c, d, t) % 3).toInt

  def part1(depth: I, target: (I, I)) = {
    memo.clear()
    (0 to target._1.toInt).map(x => (0 to target._2.toInt).map(y =>
      rtype((x, y), depth, target)).sum).sum
  }

  "part1" should "satisfy the examples given" in {
    assertResult(114)(part1(510, (10, 10)))
  }

  "part1" should "succeed" in { info(part1(4080, (14, 785)).toString) }

  def move(c: (I, I)): Set[(I, I)] =
    Set[(I, I)]((-1, 0), (0, -1), (1, 0), (0, 1)).flatMap(d =>
      if (c._1 + d._1 < 0 || c._2 + d._2 < 0) None
      else Some((c._1 + d._1, c._2 + d._2)))

  val trace = false

  val all = Set('t', 'c', 'n')
  val valid = Map(0 -> Set('c', 't'), 1 -> Set('c', 'n'), 2 -> Set('t', 'n'))

  def part2(depth: I, target: (I, I)) = {
    memo.clear()
    var me = Set[((I, I), Char)](((0, 0), 't'))
    var t = 0
    val seen = mutable.Set(me.head)
    var q = Set[((I, I), Char, Int)]()
    while (!me.contains((target, 't'))) {
      if (trace) { println(s"$t $me"); println(s"  $q") }
      val cs =
        me.flatMap(e => move(e._1).map(m => (e._1, (m, e._2)))).
          filter(e => !seen.contains(e._2))
      seen ++= cs.map(_._2)
      q ++= cs.flatMap(c => {
        val e = (rtype(c._2._1, depth, target), c._2._2)
        e match {
          case (0, 'n') | (1, 't') | (2, 'c') =>
            (all - e._2).filter(valid(rtype(c._1, depth, target)) contains _).
              map((c._1, _, 7))
          case _ => Set((c._2._1, c._2._2, 1))
        }
      })
      val qp = q.partition(_._3 == 1)
      if (trace) println(s"    $qp")
      me = qp._1.map(e => (e._1, e._2))
      q = qp._2.map(e => (e._1, e._2, e._3 - 1))
      t += 1
    }
    t
  }
  ...
  "part2" should "satisfy the examples given" in {
    assertResult(45)(part2(510, (10, 10)))
  }

  "part2" should "succeed" in { info(part2(4080, (14, 785)).toString) }
}
