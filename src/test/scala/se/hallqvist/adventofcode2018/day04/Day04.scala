package se.hallqvist.adventofcode2018.day04
...
class Day04 extends FlatSpec {
  val input = """[1518-05-08 00:12] falls asleep
                ...
                |[1518-06-08 00:40] falls asleep""".stripMargin

  abstract class Stamp(y: Int, mo: Int, d: Int, h: Int, m: Int) extends Ordered[Stamp] {
    val dt = LocalDateTime.of(y, mo, d, h, m)
    def toDateTime = dt
    def toDate = (if (h < 12) dt else dt.plusDays(1)).toLocalDate
    def compare(that: Stamp) = this.dt.compareTo(that.dt)
  }
  case class Begin(y: Int, mo: Int, d: Int, h: Int, m: Int, n: Int) extends Stamp(y, mo, d, h, m)
  case class Sleep(y: Int, mo: Int, d: Int, h: Int, m: Int) extends Stamp(y, mo, d, h, m)
  case class WakeUp(y: Int, mo: Int, d: Int, h: Int, m: Int) extends Stamp(y, mo, d, h, m)
  val beginPattern = """\[(\d+)-(\d+)-(\d+) (\d+):(\d+)\] Guard #(\d+) begins shift""".r
  val sleepPattern = """\[(\d+)-(\d+)-(\d+) (\d+):(\d+)\] falls asleep""".r
  val wakeUpPattern = """\[(\d+)-(\d+)-(\d+) (\d+):(\d+)\] wakes up""".r

  def parse(s: String) = s match {
    case beginPattern(y, mo, d, h, m, n) => new Begin(y.toInt, mo.toInt, d.toInt, h.toInt, m.toInt, n.toInt)
    case sleepPattern(y, mo, d, h, m) => new Sleep(y.toInt, mo.toInt, d.toInt, h.toInt, m.toInt)
    case wakeUpPattern(y, mo, d, h, m) => new WakeUp(y.toInt, mo.toInt, d.toInt, h.toInt, m.toInt)
  }

  // Ugly, but working way that I solved the puzzle with.
  def part1(s: String) = {
    val st = mutable.Map[Int, Long]()
    var g: Option[Int] = None
    var l: Option[Stamp] = None
    val sr = s.split("\n").map(parse).sortWith((a, b) => a.compareTo(b) < 0)
    val sc = sr.flatMap(_ match { case b: Begin => Some(b) case _ => None }).map { r => (r.toDate -> r.n) }.toMap
    val gs = sc.values.toSet
    gs.foreach(st(_) = 0)
    val gm = gs.map((g: Int) => (g, Array.fill[Int](60)(0))).toMap
    sr.foreach(_ match {
      case b: Begin => g = Some(b.n)
      case s: Sleep => l = Some(s)
      case w: WakeUp =>
        st(g.get) += ChronoUnit.MINUTES.between(l.get.toDateTime, w.toDateTime)
        (l.get.toDateTime.getMinute until w.toDateTime.getMinute).foreach(m => gm(g.get)(m) += 1)
    })
    val cg = st.maxBy(_._2)._1
    val ms = gm(cg)
    cg * ms.indexOf(ms.max)
  }

  "part1" should "satisfy the examples given" in {
    assertResult(240)(part1("""[1518-11-01 00:00] Guard #10 begins shift
                              |[1518-11-01 00:05] falls asleep
                              |[1518-11-01 00:25] wakes up
                              |[1518-11-01 00:30] falls asleep
                              |[1518-11-01 00:55] wakes up
  ...
  def part2(s: String) = {
    ...
    val mm = gm.map(_._2.max).max
    val mg = gm.find(_._2.max == mm).get._1
    mg * gm(mg).indexOf(mm)
  }

  "part2" should "satisfy the examples given" in {
    assertResult(4455)(part2("""[1518-11-01 00:00] Guard #10 begins shift
                               |[1518-11-01 00:05] falls asleep
                               |[1518-11-01 00:25] wakes up
                               |[1518-11-01 00:30] falls asleep
                               |[1518-11-01 00:55] wakes up
